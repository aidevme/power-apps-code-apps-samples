...
/**
 * Fetches entity and column metadata for the Account table via {@link AccountsService.getMetadata}.
 *
 ...
 * @example
 * ```tsx
 * const { columnDisplayNames, requiredAttributes, loading } = useAccountMetadata()
 * if (!loading) console.log(columnDisplayNames['name']) // "Account Name"
 * ```
 */
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await AccountsService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all' } })
        if (cancelled) return
        if (!result.success) {
