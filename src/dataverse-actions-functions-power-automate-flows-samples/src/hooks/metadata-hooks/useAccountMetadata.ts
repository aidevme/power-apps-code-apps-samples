// AI-CONTEXT: Fetches and derives Account entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/AccountsService.ts          (PAC-generated SDK client — do not edit)
...
// AI-CONSTRAINT: Never call AccountsService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the Account table as returned by Dataverse metadata. */
export interface IAccountAttributeMetadata {
  /** Logical column name, e.g. `"name"` or `"accountid"`. */
  logicalName: string
  /** OData schema name, e.g. `"Name"` or `"AccountId"`. */
  schemaName: string
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"account"`. */
...
/**
 * Primary key metadata for the Account table, derived from the entity metadata selects.
 *
 * @remarks
 ...
 */
export interface IAccountKeyMetadata {
...
/** A one-to-many (or many-to-one) relationship involving the Account table. */
export interface IAccountOneToManyRelationship {
...
/**
 * Privilege type for an Account table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type AccountPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the Account table. */
export interface IAccountPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateAccount"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: AccountPrivilegeType
...
/** A many-to-many relationship involving the Account table. */
export interface IAccountManyToManyRelationship {
...
/**
 * A Dataverse solution that contains the Account table as a component.
 *
 * @remarks
 ...
 */
export interface IAccountSolution {
...
/** Return value of the {@link useAccountMetadata} hook. */
export interface IUseAccountMetadataResult {
   ...
   *
   * @example `{ "name": "Account Name", "accountid": "Account", "revenue": "Annual Revenue" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the Account table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: IAccountAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: IAccountAttributeMetadata[]
  /**
   * Dataverse table type for the Account table, e.g. `"Standard"`, `"Elastic"`, `"Virtual"`, or `"Activity"`.
   * `null` while loading or on error.
   */
  tableType: string | null
  /**
   * Whether the Account table is an activity table.
   * `null` while loading or on error.
   */
  ...
  /**
   * Primary key metadata for the Account table, derived from `metadata.PrimaryIdAttribute`,
   ...
   */
  keyMetadata: IAccountKeyMetadata
  /**
   * All one-to-many relationships where Account is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: IAccountOneToManyRelationship[]
  /**
   * All many-to-one relationships where Account is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: IAccountOneToManyRelationship[]
  /**
   * All many-to-many relationships involving Account.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: IAccountManyToManyRelationship[]
  /**
   * Security privileges defined for the Account table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: IAccountPrivilege[]
  /**
   * Solutions that contain the Account table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
...
/**
 * Fetches entity and column metadata for the Dataverse `account` table via {@link AccountsService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseAccountMetadataResult} containing metadata state, per-tab derived collections,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useAccountMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['name']) // "Account Name"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "accountid"
 * ```
 */
export function useAccountMetadata(): IUseAccountMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<IAccountSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await AccountsService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useAccountMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useAccountMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useAccountMetadata: failed to load solution components', message)
  ...
  const attributes: IAccountAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: IAccountKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: IAccountOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: IAccountOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: IAccountManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: IAccountPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as AccountPrivilegeType,
