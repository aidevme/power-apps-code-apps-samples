// AI-CONTEXT: Fetches and derives AppEventLog entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/Aidevme_appeventlogsService.ts  (PAC-generated SDK client — do not edit)
...
// AI-CONSTRAINT: Never call Aidevme_appeventlogsService or fetch directly from components — all data flows through this hook.
...
export interface IAppEventLogAttributeMetadata {
  ...
  logicalName: string
  /** OData schema name, e.g. `"aidevme_Name"` or `"aidevme_EventType"`. */
  schemaName: string
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"aidevme_appeventlog"`. */
   ...
   */
  inheritsFrom: string
  /** Version string when this attribute was introduced, e.g. `"1.0.0.0"`. */
...
/**
 * Primary key metadata for the AppEventLog table, derived from the entity metadata selects.
 *
 * @remarks
 ...
 */
export interface IAppEventLogKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"aidevme_appeventlogid"`. */
  primaryIdAttribute: string | null
  /** Logical name of the primary name attribute, e.g. `"aidevme_name"`. */
  primaryNameAttribute: string | null
  /** All column logical names that form the primary key. */
  primaryKey: string[]
}

/** A one-to-many (or many-to-one) relationship involving the AppEventLog table. */
export interface IAppEventLogOneToManyRelationship {
  /** OData schema name for the relationship, e.g. `"aidevme_appeventlog_AsyncOperations"`. */
  schemaName: string
...
/**
 * Privilege type for an AppEventLog table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type AppEventLogPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the AppEventLog table. */
export interface IAppEventLogPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateaidevme_appeventlog"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: AppEventLogPrivilegeType
...
/** A many-to-many relationship involving the AppEventLog table. */
export interface IAppEventLogManyToManyRelationship {
...
/**
 * A Dataverse solution that contains the AppEventLog table as a component.
 *
 * @remarks
 ...
 */
export interface IAppEventLogSolution {
  /** Unique programmatic name of the solution, e.g. `"AiDevMeSamples"`. */
  uniqueName: string
  /** Display name shown in the maker portal, e.g. `"AiDevMe Samples"`. */
  friendlyName: string
...
export interface IUseAppEventLogMetadataResult {
  ...
  /**
   * Primary key metadata for the AppEventLog table, derived from `metadata.PrimaryIdAttribute`,
   ...
   */
  keyMetadata: IAppEventLogKeyMetadata
  /**
   * All one-to-many relationships where AppEventLog is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: IAppEventLogOneToManyRelationship[]
  /**
   * All many-to-one relationships where AppEventLog is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: IAppEventLogOneToManyRelationship[]
  /**
   * All many-to-many relationships involving AppEventLog.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: IAppEventLogManyToManyRelationship[]
  /**
   * Security privileges defined for the AppEventLog table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: IAppEventLogPrivilege[]
  /**
   * Solutions that contain the AppEventLog table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
   ...
   */
  solutions: IAppEventLogSolution[]
...
/**
 * Fetches entity and column metadata for the Dataverse `aidevme_appeventlog` table via {@link Aidevme_appeventlogsService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseAppEventLogMetadataResult} containing metadata state, per-tab derived collections,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useAppEventLogMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['aidevme_name']) // "Name"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "aidevme_appeventlogid"
 * ```
 */
export function useAppEventLogMetadata(): IUseAppEventLogMetadataResult {
  ...
  const [solutions, setSolutions] = useState<IAppEventLogSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await Aidevme_appeventlogsService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useAppEventLogMetadata: failed to load solution components', message)
  ...
  const keyMetadata: IAppEventLogKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: IAppEventLogOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: IAppEventLogOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: IAppEventLogManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: IAppEventLogPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as AppEventLogPrivilegeType,
