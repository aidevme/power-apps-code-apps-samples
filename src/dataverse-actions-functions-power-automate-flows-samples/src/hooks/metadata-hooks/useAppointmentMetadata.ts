// AI-CONTEXT: Fetches and derives Appointment entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/AppointmentsService.ts         (PAC-generated SDK client — do not edit)
...
// AI-CONSTRAINT: Never call AppointmentsService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the Appointment table as returned by Dataverse metadata. */
export interface IAppointmentAttributeMetadata {
  /** Logical column name, e.g. `"activityid"` or `"subject"`. */
  logicalName: string
  /** OData schema name, e.g. `"ActivityId"` or `"Subject"`. */
  schemaName: string
  /** User-localised display label for the column. */
  displayName: string
  /**
   * Dataverse attribute type name, e.g. `"StringType"`, `"DateTimeType"`, `"IntegerType"`.
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"appointment"`. */
...
/**
 * Primary key metadata for the Appointment table, derived from the entity metadata selects.
 *
 * @remarks
 ...
 */
export interface IAppointmentKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"activityid"`. */
  primaryIdAttribute: string | null
  /** Logical name of the primary name attribute, e.g. `"subject"`. */
  primaryNameAttribute: string | null
  /** All column logical names that form the primary key. */
  primaryKey: string[]
}

/** A one-to-many (or many-to-one) relationship involving the Appointment table. */
export interface IAppointmentOneToManyRelationship {
  /** OData schema name for the relationship, e.g. `"Appointment_ActivityPointers"`. */
  schemaName: string
...
/**
 * Privilege type for an Appointment table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type AppointmentPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the Appointment table. */
export interface IAppointmentPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateAppointment"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: AppointmentPrivilegeType
...
/** A many-to-many relationship involving the Appointment table. */
export interface IAppointmentManyToManyRelationship {
...
/**
 * A Dataverse solution that contains the Appointment table as a component.
 *
 * @remarks
 ...
 */
export interface IAppointmentSolution {
...
/** Return value of the {@link useAppointmentMetadata} hook. */
export interface IUseAppointmentMetadataResult {
   ...
   *
   * @example `{ "subject": "Subject", "activityid": "Appointment", "scheduledstart": "Start Time" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the Appointment table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: IAppointmentAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: IAppointmentAttributeMetadata[]
  /**
   * Dataverse table type for the Appointment table, e.g. `"Standard"`, `"Activity"`.
   * `null` while loading or on error.
   */
  tableType: string | null
  /**
   * Whether the Appointment table is an activity table.
   * `null` while loading or on error.
   */
  ...
  /**
   * Primary key metadata for the Appointment table, derived from `metadata.PrimaryIdAttribute`,
   ...
   */
  keyMetadata: IAppointmentKeyMetadata
  /**
   * All one-to-many relationships where Appointment is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: IAppointmentOneToManyRelationship[]
  /**
   * All many-to-one relationships where Appointment is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: IAppointmentOneToManyRelationship[]
  /**
   * All many-to-many relationships involving Appointment.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: IAppointmentManyToManyRelationship[]
  /**
   * Security privileges defined for the Appointment table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: IAppointmentPrivilege[]
  /**
   * Solutions that contain the Appointment table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
   ...
   */
  solutions: IAppointmentSolution[]
...
/**
 * Fetches entity and column metadata for the Dataverse `appointment` table via {@link AppointmentsService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseAppointmentMetadataResult} containing metadata state, per-tab derived collections,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useAppointmentMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['subject']) // "Subject"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "activityid"
 * ```
 */
export function useAppointmentMetadata(): IUseAppointmentMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<IAppointmentSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await AppointmentsService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useAppointmentMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useAppointmentMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useAppointmentMetadata: failed to load solution components', message)
  ...
  const attributes: IAppointmentAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: IAppointmentKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: IAppointmentOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: IAppointmentOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: IAppointmentManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: IAppointmentPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as AppointmentPrivilegeType,
