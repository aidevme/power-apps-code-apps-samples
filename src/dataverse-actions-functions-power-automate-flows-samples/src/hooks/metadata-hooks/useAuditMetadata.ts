// AI-CONTEXT: Fetches and derives Audit entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/AuditsService.ts             (PAC-generated SDK client — do not edit)
...
/** Describes a single attribute on the Audit table as returned by Dataverse metadata. */
export interface IAuditAttributeMetadata {
  /** Logical column name, e.g. `"auditid"` or `"action"`. */
  logicalName: string
  /** OData schema name, e.g. `"AuditId"` or `"Action"`. */
  schemaName: string
  /** User-localised display label for the column. */
  displayName: string
  /**
   * Dataverse attribute type name, e.g. `"UniqueidentifierType"`, `"PicklistType"`, `"DateTimeType"`.
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"audit"`. */
...
/**
 * Primary key metadata for the Audit table, derived from the entity metadata selects.
 *
 * @remarks
 ...
 */
export interface IAuditKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"auditid"`. */
...
/** A one-to-many (or many-to-one) relationship involving the Audit table. */
export interface IAuditOneToManyRelationship {
...
/**
 * Privilege type for an Audit table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type AuditPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the Audit table. */
export interface IAuditPrivilege {
  /** Internal name of the privilege, e.g. `"prvReadAudit"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: AuditPrivilegeType
...
/** A many-to-many relationship involving the Audit table. */
export interface IAuditManyToManyRelationship {
  /** OData schema name for the relationship. */
  schemaName: string
...
/** Return value of the {@link useAuditMetadata} hook. */
export interface IUseAuditMetadataResult {
   ...
   *
   * @example `{ "auditid": "Audit", "action": "Event", "objectid": "Record" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the Audit table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: IAuditAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: IAuditAttributeMetadata[]
  /**
   * Dataverse table type for the Audit table, e.g. `"Standard"` or `"Virtual"`.
   * `null` while loading or on error.
   */
  tableType: string | null
  /**
   * Whether the Audit table is an activity table.
   * `null` while loading or on error.
   */
  ...
  /**
   * Primary key metadata for the Audit table, derived from `metadata.PrimaryIdAttribute`,
   * `metadata.PrimaryNameAttribute`, and `metadata.PrimaryKey`.
   * All fields are `null` / empty while loading or on error.
   */
  keyMetadata: IAuditKeyMetadata
  /**
   * All one-to-many relationships where Audit is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: IAuditOneToManyRelationship[]
  /**
   * All many-to-one relationships where Audit is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: IAuditOneToManyRelationship[]
  /**
   * All many-to-many relationships involving Audit.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: IAuditManyToManyRelationship[]
  /**
   * Security privileges defined for the Audit table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: IAuditPrivilege[]
  /**
   * Solutions that contain the Audit table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
...
/**
 * Fetches entity and column metadata for the Dataverse `audit` table via {@link AuditsService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * All derived collections return empty arrays while loading or on error.
 *
 * @returns {@link IUseAuditMetadataResult} containing metadata state, derived collections,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useAuditMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['action']) // "Event"
 * ```
 */
export function useAuditMetadata(): IUseAuditMetadataResult {
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await AuditsService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useAuditMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useAuditMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  const attributes: IAuditAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: IAuditKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: IAuditOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: IAuditOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: IAuditManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: IAuditPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as AuditPrivilegeType,
