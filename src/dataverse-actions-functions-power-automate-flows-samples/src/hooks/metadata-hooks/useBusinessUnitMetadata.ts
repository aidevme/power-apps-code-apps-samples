// AI-CONTEXT: Fetches and derives BusinessUnit entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/BusinessunitsService.ts        (PAC-generated SDK client — do not edit)
...
// AI-CONSTRAINT: Never call BusinessunitsService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the BusinessUnit table as returned by Dataverse metadata. */
export interface IBusinessUnitAttributeMetadata {
  /** Logical column name, e.g. `"name"` or `"businessunitid"`. */
  logicalName: string
  /** OData schema name, e.g. `"Name"` or `"BusinessUnitId"`. */
  schemaName: string
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"businessunit"`. */
...
/**
 * Primary key metadata for the BusinessUnit table, derived from the entity metadata selects.
 *
 * @remarks
 ...
 */
export interface IBusinessUnitKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"businessunitid"`. */
...
/** A one-to-many (or many-to-one) relationship involving the BusinessUnit table. */
export interface IBusinessUnitOneToManyRelationship {
  /** OData schema name for the relationship, e.g. `"business_unit_accounts"`. */
  schemaName: string
...
/**
 * Privilege type for a BusinessUnit table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type BusinessUnitPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the BusinessUnit table. */
export interface IBusinessUnitPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateBusinessUnit"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: BusinessUnitPrivilegeType
...
/** A many-to-many relationship involving the BusinessUnit table. */
export interface IBusinessUnitManyToManyRelationship {
...
/**
 * A Dataverse solution that contains the BusinessUnit table as a component.
 *
 * @remarks
 ...
 */
export interface IBusinessUnitSolution {
...
/** Return value of the {@link useBusinessUnitMetadata} hook. */
export interface IUseBusinessUnitMetadataResult {
   ...
   *
   * @example `{ "name": "Name", "businessunitid": "Business Unit", "parentbusinessunitid": "Parent Business" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the BusinessUnit table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: IBusinessUnitAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: IBusinessUnitAttributeMetadata[]
  /**
   * Dataverse table type for the BusinessUnit table, e.g. `"Standard"`.
   * `null` while loading or on error.
   */
  tableType: string | null
  /**
   * Whether the BusinessUnit table is an activity table.
   * `null` while loading or on error.
   */
  ...
  /**
   * Primary key metadata for the BusinessUnit table, derived from `metadata.PrimaryIdAttribute`,
   ...
   */
  keyMetadata: IBusinessUnitKeyMetadata
  /**
   * All one-to-many relationships where BusinessUnit is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: IBusinessUnitOneToManyRelationship[]
  /**
   * All many-to-one relationships where BusinessUnit is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: IBusinessUnitOneToManyRelationship[]
  /**
   * All many-to-many relationships involving BusinessUnit.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: IBusinessUnitManyToManyRelationship[]
  /**
   * Security privileges defined for the BusinessUnit table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: IBusinessUnitPrivilege[]
  /**
   * Solutions that contain the BusinessUnit table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
   ...
   */
  solutions: IBusinessUnitSolution[]
...
/**
 * Fetches entity and column metadata for the Dataverse `businessunit` table via {@link BusinessunitsService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseBusinessUnitMetadataResult} containing metadata state, per-tab derived collections,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useBusinessUnitMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['name']) // "Name"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "businessunitid"
 * ```
 */
export function useBusinessUnitMetadata(): IUseBusinessUnitMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<IBusinessUnitSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await BusinessunitsService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useBusinessUnitMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useBusinessUnitMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useBusinessUnitMetadata: failed to load solution components', message)
  ...
  const attributes: IBusinessUnitAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: IBusinessUnitKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: IBusinessUnitOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: IBusinessUnitOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: IBusinessUnitManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: IBusinessUnitPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as BusinessUnitPrivilegeType,
