// AI-CONTEXT: Fetches and derives ConfigurationSetting entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/Aidevme_codeappssamplesconfigurationsettingsService.ts  (PAC-generated SDK client — do not edit)
//   - types:    src/hooks/metadata-hooks/entityMetadata.types.ts                               (IEntityTableInfo, ALL_ENTITY_METADATA_FIELDS, deriveTableInfo)
//   - consumer: src/components/apps/miscellaneous/metadata/EntityDetailsApp.tsx                (primary caller)
// AI-CONSTRAINT: Never call Aidevme_codeappssamplesconfigurationsettingsService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the ConfigurationSetting table as returned by Dataverse metadata. */
export interface IConfigurationSettingAttributeMetadata {
  /** Logical column name, e.g. `"aidevme_key"` or `"aidevme_codeappssamplesconfigurationsettingid"`. */
  logicalName: string
  /** OData schema name, e.g. `"aidevme_Key"` or `"aidevme_codeappssamplesconfigurationsettingId"`. */
  schemaName: string
  /** User-localised display label for the column. */
  displayName: string
  /**
   * Dataverse attribute type name, e.g. `"StringType"`, `"MemoType"`, `"BooleanType"`.
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"aidevme_codeappssamplesconfigurationsettings"`. */
...
/**
 * Primary key metadata for the ConfigurationSetting table, derived from the entity metadata selects.
 *
 * @remarks
 * Dataverse table: `aidevme_codeappssamplesconfigurationsettings` (publisher prefix: `aidevme_`).
 ...
 */
export interface IConfigurationSettingKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"aidevme_codeappssamplesconfigurationsettingid"`. */
  primaryIdAttribute: string | null
  /** Logical name of the primary name attribute, e.g. `"aidevme_key"`. */
  primaryNameAttribute: string | null
  /** All column logical names that form the primary key. */
  primaryKey: string[]
}

/** A one-to-many (or many-to-one) relationship involving the ConfigurationSetting table. */
export interface IConfigurationSettingOneToManyRelationship {
...
/**
 * Privilege type for a ConfigurationSetting table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type ConfigurationSettingPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the ConfigurationSetting table. */
export interface IConfigurationSettingPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateaidevme_codeappssamplesconfigurationsettings"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: ConfigurationSettingPrivilegeType
...
/** A many-to-many relationship involving the ConfigurationSetting table. */
export interface IConfigurationSettingManyToManyRelationship {
  /** OData schema name for the relationship. */
  schemaName: string
...
/**
 * A Dataverse solution that contains the ConfigurationSetting table as a component.
 *
 * @remarks
 ...
 */
export interface IConfigurationSettingSolution {
  /** Unique programmatic name of the solution, e.g. `"aidevme_CodeAppsSamples"`. */
  uniqueName: string
...
/** Return value of the {@link useConfigurationSettingMetadata} hook. */
export interface IUseConfigurationSettingMetadataResult {
   ...
   *
   * @example `{ "aidevme_key": "Key", "aidevme_textvalue": "Text Value" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the ConfigurationSetting table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: IConfigurationSettingAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: IConfigurationSettingAttributeMetadata[]
  /**
   * Dataverse table type for the ConfigurationSetting table, e.g. `"Standard"`.
   * `null` while loading or on error.
   */
  tableType: string | null
  /**
   * Whether the ConfigurationSetting table is an activity table.
   * `null` while loading or on error.
   */
  ...
  /**
   * Primary key metadata for the ConfigurationSetting table.
   * All fields are `null` / empty while loading or on error.
   *
   * @remarks
   * Dataverse table: `aidevme_codeappssamplesconfigurationsettings` (publisher prefix: `aidevme_`).
   * The SDK does not expose alternate key metadata — only primary key fields are available.
   */
  keyMetadata: IConfigurationSettingKeyMetadata
  /**
   * All one-to-many relationships where ConfigurationSetting is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: IConfigurationSettingOneToManyRelationship[]
  /**
   * All many-to-one relationships where ConfigurationSetting is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: IConfigurationSettingOneToManyRelationship[]
  /**
   * All many-to-many relationships involving ConfigurationSetting.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: IConfigurationSettingManyToManyRelationship[]
  /**
   * Security privileges defined for the ConfigurationSetting table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: IConfigurationSettingPrivilege[]
  /**
   * Solutions that contain the ConfigurationSetting table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
   ...
   */
  solutions: IConfigurationSettingSolution[]
...
/**
 * Fetches entity and column metadata for the Dataverse `aidevme_codeappssamplesconfigurationsettings` table
 * via {@link Aidevme_codeappssamplesconfigurationsettingsService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseConfigurationSettingMetadataResult} containing metadata state, per-tab derived collections,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useConfigurationSettingMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['aidevme_key']) // "Key"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "aidevme_codeappssamplesconfigurationsettingid"
 * ```
 */
export function useConfigurationSettingMetadata(): IUseConfigurationSettingMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<IConfigurationSettingSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await Aidevme_codeappssamplesconfigurationsettingsService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useConfigurationSettingMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useConfigurationSettingMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useConfigurationSettingMetadata: failed to load solution components', message)
  ...
  const attributes: IConfigurationSettingAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: IConfigurationSettingKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: IConfigurationSettingOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: IConfigurationSettingOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: IConfigurationSettingManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: IConfigurationSettingPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as ConfigurationSettingPrivilegeType,
