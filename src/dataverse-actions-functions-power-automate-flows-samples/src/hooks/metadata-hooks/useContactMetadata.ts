...
export interface IContactAttributeMetadata {
  ...
  displayName: string
  /** Dataverse attribute type, e.g. `"StringType"`, `"MoneyType"`, `"DateTimeType"`. */
...
/**
 * Fetches entity and column metadata for the Contact table via {@link ContactsService.getMetadata}.
 *
 ...
 * @example
 * ```tsx
 * const { columnDisplayNames, requiredAttributes, loading } = useContactMetadata()
 * if (!loading) console.log(columnDisplayNames['fullname']) // "Full Name"
 * ```
 */
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await ContactsService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all' } })
        if (cancelled) return
        if (!result.success) {
