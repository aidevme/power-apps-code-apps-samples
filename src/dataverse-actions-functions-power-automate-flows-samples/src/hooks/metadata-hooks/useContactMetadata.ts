// AI-CONTEXT: Fetches and derives Contact entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/ContactsService.ts          (PAC-generated SDK client — do not edit)
...
// AI-CONSTRAINT: Never call ContactsService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the Contact table as returned by Dataverse metadata. */
export interface IContactAttributeMetadata {
  /** Logical column name, e.g. `"fullname"` or `"contactid"`. */
  logicalName: string
  /** OData schema name, e.g. `"FullName"` or `"ContactId"`. */
  schemaName: string
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"contact"`. */
  ...
  /** Whether the column is marked as required on Dataverse forms (`IsRequiredForForm`). */
  ...
  metadataId: string
  /** Whether customisations can be applied to this column (`IsCustomizable.Value`). */
  isCustomizable: boolean
  /** Whether this column holds a data source secret. */
  ...
  /** Whether this is a logical attribute (not stored directly in the database). */
  isLogical: boolean
  /** Whether this column can be renamed (`IsRenameable.Value`). */
  isRenameable: boolean
  /** Whether this column is retrievable via the Web API (`IsRetrievable`). */
  isRetrievable: boolean
  /** Whether sort is enabled for this column (`IsSortableEnabled.Value`). */
...
/**
 * Primary key metadata for the Contact table, derived from the entity metadata selects.
 *
 * @remarks
 ...
 */
export interface IContactKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"contactid"`. */
  primaryIdAttribute: string | null
  /** Logical name of the primary name attribute, e.g. `"fullname"`. */
  primaryNameAttribute: string | null
  /** All column logical names that form the primary key. */
  primaryKey: string[]
}

/** A one-to-many (or many-to-one) relationship involving the Contact table. */
export interface IContactOneToManyRelationship {
  /** OData schema name for the relationship, e.g. `"contact_customer_accounts"`. */
  schemaName: string
...
/**
 * Privilege type for a Contact table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type ContactPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the Contact table. */
export interface IContactPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateContact"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: ContactPrivilegeType
...
/** A many-to-many relationship involving the Contact table. */
export interface IContactManyToManyRelationship {
...
/**
 * A Dataverse solution that contains the Contact table as a component.
 *
 * @remarks
 ...
 */
export interface IContactSolution {
...
/** Return value of the {@link useContactMetadata} hook. */
export interface IUseContactMetadataResult {
   ...
   *
   * @example `{ "fullname": "Full Name", "contactid": "Contact", "emailaddress1": "Email" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the Contact table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: IContactAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: IContactAttributeMetadata[]
  /**
   * Dataverse table type for the Contact table, e.g. `"Standard"`, `"Elastic"`, `"Virtual"`, or `"Activity"`.
   * `null` while loading or on error.
   */
  tableType: string | null
  /**
   * Whether the Contact table is an activity table.
   * `null` while loading or on error.
   */
  ...
  /**
   * Primary key metadata for the Contact table, derived from `metadata.PrimaryIdAttribute`,
   ...
   */
  keyMetadata: IContactKeyMetadata
  /**
   * All one-to-many relationships where Contact is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: IContactOneToManyRelationship[]
  /**
   * All many-to-one relationships where Contact is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: IContactOneToManyRelationship[]
  /**
   * All many-to-many relationships involving Contact.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: IContactManyToManyRelationship[]
  /**
   * Security privileges defined for the Contact table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: IContactPrivilege[]
  /**
   * Solutions that contain the Contact table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
...
/**
 * Fetches entity and column metadata for the Dataverse `contact` table via {@link ContactsService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseContactMetadataResult} containing metadata state, per-tab derived collections,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useContactMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['fullname']) // "Full Name"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "contactid"
 * ```
 */
export function useContactMetadata(): IUseContactMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<IContactSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await ContactsService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useContactMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useContactMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useContactMetadata: failed to load solution components', message)
  ...
  const attributes: IContactAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: IContactKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: IContactOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: IContactOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: IContactManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: IContactPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as ContactPrivilegeType,
