// AI-CONTEXT: Fetches and derives Custom API entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
...
// AI-CONSTRAINT: Never call CustomapisService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the Custom API table as returned by Dataverse metadata. */
export interface ICustomApiAttributeMetadata {
  /** Logical column name, e.g. `"uniquename"` or `"customapiid"`. */
  logicalName: string
  /** OData schema name, e.g. `"UniqueName"` or `"CustomAPIId"`. */
  schemaName: string
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"customapi"`. */
...
/**
 * Primary key metadata for the Custom API table, derived from the entity metadata selects.
 *
 * @remarks
 ...
 */
export interface ICustomApiKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"customapiid"`. */
...
/** A one-to-many (or many-to-one) relationship involving the Custom API table. */
export interface ICustomApiOneToManyRelationship {
...
/**
 * Privilege type for a Custom API table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type CustomApiPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the Custom API table. */
export interface ICustomApiPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateCustomAPI"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: CustomApiPrivilegeType
...
/** A many-to-many relationship involving the Custom API table. */
export interface ICustomApiManyToManyRelationship {
  /** OData schema name for the relationship. */
  schemaName: string
...
/**
 * A Dataverse solution that contains the Custom API table as a component.
 *
 * @remarks
 ...
 */
export interface ICustomApiSolution {
...
/** Return value of the {@link useCustomApiMetadata} hook. */
export interface IUseCustomApiMetadataResult {
   ...
   *
   * @example `{ "uniquename": "Unique Name", "customapiid": "Custom API", "displayname": "Display Name" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the Custom API table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: ICustomApiAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: ICustomApiAttributeMetadata[]
  /**
   * Dataverse table type for the Custom API table, e.g. `"Standard"`.
   * `null` while loading or on error.
   */
  tableType: string | null
  /**
   * Whether the Custom API table is an activity table.
   * `null` while loading or on error.
   */
  ...
  /**
   * Primary key metadata for the Custom API table, derived from `metadata.PrimaryIdAttribute`,
   ...
   */
  keyMetadata: ICustomApiKeyMetadata
  /**
   * All one-to-many relationships where Custom API is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: ICustomApiOneToManyRelationship[]
  /**
   * All many-to-one relationships where Custom API is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: ICustomApiOneToManyRelationship[]
  /**
   * All many-to-many relationships involving Custom API.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: ICustomApiManyToManyRelationship[]
  /**
   * Security privileges defined for the Custom API table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: ICustomApiPrivilege[]
  /**
   * Solutions that contain the Custom API table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
...
/**
 * Fetches entity and column metadata for the Dataverse `customapi` table via {@link CustomapisService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseCustomApiMetadataResult} containing metadata state, per-tab derived collections,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useCustomApiMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['uniquename']) // "Unique Name"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "customapiid"
 * ```
 */
export function useCustomApiMetadata(): IUseCustomApiMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<ICustomApiSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await CustomapisService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useCustomApiMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useCustomApiMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useCustomApiMetadata: failed to load solution components', message)
  ...
  const attributes: ICustomApiAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: ICustomApiKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: ICustomApiOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: ICustomApiOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: ICustomApiManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: ICustomApiPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as CustomApiPrivilegeType,
