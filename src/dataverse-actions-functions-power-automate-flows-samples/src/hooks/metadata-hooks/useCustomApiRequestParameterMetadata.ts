// AI-CONTEXT: Fetches and derives Custom API Request Parameter entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
...
// AI-CONSTRAINT: Never call CustomapirequestparametersService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the Custom API Request Parameter table as returned by Dataverse metadata. */
export interface ICustomApiRequestParameterAttributeMetadata {
  /** Logical column name, e.g. `"uniquename"` or `"customapirequestparameterid"`. */
  logicalName: string
  /** OData schema name, e.g. `"UniqueName"` or `"CustomAPIRequestParameterId"`. */
  schemaName: string
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"customapirequestparameter"`. */
...
/**
 * Primary key metadata for the Custom API Request Parameter table, derived from the entity metadata selects.
 *
 * @remarks
 ...
 */
export interface ICustomApiRequestParameterKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"customapirequestparameterid"`. */
...
/** A one-to-many (or many-to-one) relationship involving the Custom API Request Parameter table. */
export interface ICustomApiRequestParameterOneToManyRelationship {
...
/**
 * Privilege type for a Custom API Request Parameter table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type CustomApiRequestParameterPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the Custom API Request Parameter table. */
export interface ICustomApiRequestParameterPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateCustomAPIRequestParameter"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: CustomApiRequestParameterPrivilegeType
...
/** A many-to-many relationship involving the Custom API Request Parameter table. */
export interface ICustomApiRequestParameterManyToManyRelationship {
  /** OData schema name for the relationship. */
  schemaName: string
...
/**
 * A Dataverse solution that contains the Custom API Request Parameter table as a component.
 *
 * @remarks
 ...
 */
export interface ICustomApiRequestParameterSolution {
...
/** Return value of the {@link useCustomApiRequestParameterMetadata} hook. */
export interface IUseCustomApiRequestParameterMetadataResult {
   ...
   *
   * @example `{ "uniquename": "Unique Name", "customapirequestparameterid": "Custom API Request Parameter" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the Custom API Request Parameter table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: ICustomApiRequestParameterAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: ICustomApiRequestParameterAttributeMetadata[]
  /**
   * Dataverse table type for the Custom API Request Parameter table, e.g. `"Standard"`.
   * `null` while loading or on error.
   */
  tableType: string | null
  /**
   * Whether the Custom API Request Parameter table is an activity table.
   * `null` while loading or on error.
   */
  ...
  /**
   * Primary key metadata for the Custom API Request Parameter table, derived from
   ...
   */
  keyMetadata: ICustomApiRequestParameterKeyMetadata
  /**
   * All one-to-many relationships where Custom API Request Parameter is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: ICustomApiRequestParameterOneToManyRelationship[]
  /**
   * All many-to-one relationships where Custom API Request Parameter is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: ICustomApiRequestParameterOneToManyRelationship[]
  /**
   * All many-to-many relationships involving Custom API Request Parameter.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: ICustomApiRequestParameterManyToManyRelationship[]
  /**
   * Security privileges defined for the Custom API Request Parameter table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: ICustomApiRequestParameterPrivilege[]
  /**
   * Solutions that contain the Custom API Request Parameter table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
   ...
   */
  solutions: ICustomApiRequestParameterSolution[]
...
/**
 * Fetches entity and column metadata for the Dataverse `customapirequestparameter` table
 * via {@link CustomapirequestparametersService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseCustomApiRequestParameterMetadataResult} containing metadata state, per-tab derived
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useCustomApiRequestParameterMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['uniquename']) // "Unique Name"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "customapirequestparameterid"
 * ```
 */
export function useCustomApiRequestParameterMetadata(): IUseCustomApiRequestParameterMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<ICustomApiRequestParameterSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await CustomapirequestparametersService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useCustomApiRequestParameterMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useCustomApiRequestParameterMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useCustomApiRequestParameterMetadata: failed to load solution components', message)
  ...
  const attributes: ICustomApiRequestParameterAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: ICustomApiRequestParameterKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: ICustomApiRequestParameterOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: ICustomApiRequestParameterOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: ICustomApiRequestParameterManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: ICustomApiRequestParameterPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as CustomApiRequestParameterPrivilegeType,
