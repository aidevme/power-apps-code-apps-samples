// AI-CONTEXT: Fetches and derives Custom API Response Property entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
...
// AI-CONSTRAINT: Never call CustomapiresponsepropertiesService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the Custom API Response Property table as returned by Dataverse metadata. */
export interface ICustomApiResponsePropertyAttributeMetadata {
  /** Logical column name, e.g. `"uniquename"` or `"customapiresponsepropertyid"`. */
  logicalName: string
  /** OData schema name, e.g. `"UniqueName"` or `"CustomAPIResponsePropertyId"`. */
  schemaName: string
  /** User-localised display label for the column. */
  displayName: string
  /**
   * Dataverse attribute type name, e.g. `"StringType"`, `"BooleanType"`, `"LookupType"`.
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"customapiresponseproperty"`. */
  ...
  /** Whether the column can be customised in a solution (`IsCustomizable.Value`). */
  isCustomizable: boolean
  /** Whether the column stores a data source secret (e.g. a connection string). */
  isDataSourceSecret: boolean
  /** Whether the column can be used as a global filter in interactive experience dashboards (`IsGlobalFilterEnabled.Value`). */
  isGlobalFilterEnabled: boolean
  /** Whether this is a logical attribute that is not stored directly in the table. */
  isLogical: boolean
  /** Whether the column display name can be changed (`IsRenameable.Value`). */
  isRenameable: boolean
  /** Whether the column is returned by the retrieve operation (`IsRetrievable`). */
  isRetrievable: boolean
  /** Whether the column can be used as a sort column in views (`IsSortableEnabled.Value`). */
...
/**
 * Primary key metadata for the Custom API Response Property table, derived from the entity metadata selects.
 *
 * @remarks
 ...
 */
export interface ICustomApiResponsePropertyKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"customapiresponsepropertyid"`. */
  primaryIdAttribute: string | null
  /** Logical name of the primary name attribute, e.g. `"uniquename"`. */
  primaryNameAttribute: string | null
  /** All column logical names that form the primary key. */
  primaryKey: string[]
}

/** A one-to-many (or many-to-one) relationship involving the Custom API Response Property table. */
export interface ICustomApiResponsePropertyOneToManyRelationship {
...
/**
 * Privilege type for a Custom API Response Property table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type CustomApiResponsePropertyPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the Custom API Response Property table. */
export interface ICustomApiResponsePropertyPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateCustomAPIResponseProperty"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: CustomApiResponsePropertyPrivilegeType
...
/** A many-to-many relationship involving the Custom API Response Property table. */
export interface ICustomApiResponsePropertyManyToManyRelationship {
  /** OData schema name for the relationship. */
  schemaName: string
...
/**
 * A Dataverse solution that contains the Custom API Response Property table as a component.
 *
 * @remarks
 ...
 */
export interface ICustomApiResponsePropertySolution {
...
/** Return value of the {@link useCustomApiResponsePropertyMetadata} hook. */
export interface IUseCustomApiResponsePropertyMetadataResult {
   ...
   *
   * @example `{ "uniquename": "Unique Name", "customapiresponsepropertyid": "Custom API Response Property" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the Custom API Response Property table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: ICustomApiResponsePropertyAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: ICustomApiResponsePropertyAttributeMetadata[]
  /**
   * Dataverse table type for the Custom API Response Property table, e.g. `"Standard"`.
   * `null` while loading or on error.
   */
  tableType: string | null
  /**
   * Whether the Custom API Response Property table is an activity table.
   * `null` while loading or on error.
   */
  ...
  /**
   * Primary key metadata for the Custom API Response Property table, derived from
   * `metadata.PrimaryIdAttribute`, `metadata.PrimaryNameAttribute`, and `metadata.PrimaryKey`.
   ...
   */
  keyMetadata: ICustomApiResponsePropertyKeyMetadata
  /**
   * All one-to-many relationships where Custom API Response Property is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: ICustomApiResponsePropertyOneToManyRelationship[]
  /**
   * All many-to-one relationships where Custom API Response Property is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: ICustomApiResponsePropertyOneToManyRelationship[]
  /**
   * All many-to-many relationships involving Custom API Response Property.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: ICustomApiResponsePropertyManyToManyRelationship[]
  /**
   * Security privileges defined for the Custom API Response Property table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: ICustomApiResponsePropertyPrivilege[]
  /**
   * Solutions that contain the Custom API Response Property table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
   ...
   */
  solutions: ICustomApiResponsePropertySolution[]
...
/**
 * Fetches entity and column metadata for the Dataverse `customapiresponseproperty` table
 * via {@link CustomapiresponsepropertiesService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseCustomApiResponsePropertyMetadataResult} containing metadata state, per-tab derived
 * collections, loading/error flags for both fetch effects, and a `retry` callback for error recovery.
 *
 * @throws Never — errors are caught internally and surfaced via `error` or `solutionsError`.
 *
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useCustomApiResponsePropertyMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['uniquename']) // "Unique Name"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "customapiresponsepropertyid"
 * ```
 */
export function useCustomApiResponsePropertyMetadata(): IUseCustomApiResponsePropertyMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<ICustomApiResponsePropertySolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await CustomapiresponsepropertiesService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useCustomApiResponsePropertyMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useCustomApiResponsePropertyMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useCustomApiResponsePropertyMetadata: failed to load solution components', message)
  ...
  const attributes: ICustomApiResponsePropertyAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: ICustomApiResponsePropertyKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: ICustomApiResponsePropertyOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: ICustomApiResponsePropertyOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: ICustomApiResponsePropertyManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: ICustomApiResponsePropertyPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as CustomApiResponsePropertyPrivilegeType,
