// AI-CONTEXT: Fetches and derives Email entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/EmailsService.ts               (PAC-generated SDK client — do not edit)
...
// AI-CONSTRAINT: Never call EmailsService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the Email table as returned by Dataverse metadata. */
export interface IEmailAttributeMetadata {
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"email"`. */
...
/**
 * Primary key metadata for the Email table, derived from the entity metadata selects.
 *
 * @remarks
 ...
 */
export interface IEmailKeyMetadata {
...
/** A one-to-many (or many-to-one) relationship involving the Email table. */
export interface IEmailOneToManyRelationship {
  /** OData schema name for the relationship, e.g. `"email_activity_parties"`. */
  schemaName: string
...
/**
 * Privilege type for an Email table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type EmailPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the Email table. */
export interface IEmailPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateEmail"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: EmailPrivilegeType
...
/** A many-to-many relationship involving the Email table. */
export interface IEmailManyToManyRelationship {
...
/**
 * A Dataverse solution that contains the Email table as a component.
 *
 * @remarks
 ...
 */
export interface IEmailSolution {
...
/** Return value of the {@link useEmailMetadata} hook. */
export interface IUseEmailMetadataResult {
   ...
   *
   * @example `{ "subject": "Subject", "activityid": "Email", "description": "Description" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the Email table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: IEmailAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: IEmailAttributeMetadata[]
  /**
   * Dataverse table type for the Email table, e.g. `"Standard"`, `"Activity"`.
   * `null` while loading or on error.
   */
  tableType: string | null
  /**
   * Whether the Email table is an activity table.
   * `null` while loading or on error.
   */
  ...
  /**
   * Primary key metadata for the Email table, derived from `metadata.PrimaryIdAttribute`,
   ...
   */
  keyMetadata: IEmailKeyMetadata
  /**
   * All one-to-many relationships where Email is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: IEmailOneToManyRelationship[]
  /**
   * All many-to-one relationships where Email is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: IEmailOneToManyRelationship[]
  /**
   * All many-to-many relationships involving Email.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: IEmailManyToManyRelationship[]
  /**
   * Security privileges defined for the Email table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: IEmailPrivilege[]
  /**
   * Solutions that contain the Email table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
...
/**
 * Fetches entity and column metadata for the Dataverse `email` table via {@link EmailsService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseEmailMetadataResult} containing metadata state, per-tab derived collections,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useEmailMetadata()
 ...
 * ```
 */
export function useEmailMetadata(): IUseEmailMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<IEmailSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await EmailsService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useEmailMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useEmailMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useEmailMetadata: failed to load solution components', message)
  ...
  const attributes: IEmailAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: IEmailKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: IEmailOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: IEmailOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: IEmailManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: IEmailPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as EmailPrivilegeType,
