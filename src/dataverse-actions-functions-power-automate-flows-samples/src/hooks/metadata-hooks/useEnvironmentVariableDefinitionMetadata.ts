...
export function useEnvironmentVariableDefinitionMetadata(): IUseEnvironmentVariableDefinitionMetadataResult {
  ...
  const [solutions, setSolutions] = useState<IEnvironmentVariableDefinitionSolution[]>([])
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useEnvironmentVariableDefinitionMetadata: failed to load solution components', message)
