// AI-CONTEXT: Fetches and derives EnvironmentVariableValue entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/EnvironmentvariablevaluesService.ts  (PAC-generated SDK client — do not edit)
...
// AI-CONSTRAINT: Never call EnvironmentvariablevaluesService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the EnvironmentVariableValue table as returned by Dataverse metadata. */
export interface IEnvironmentVariableValueAttributeMetadata {
  /** Logical column name, e.g. `"value"` or `"environmentvariablevalueid"`. */
  logicalName: string
  /** OData schema name, e.g. `"Value"` or `"EnvironmentVariableValueId"`. */
  schemaName: string
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"environmentvariablevalue"`. */
...
/**
 * Primary key metadata for the EnvironmentVariableValue table.
 *
 * @remarks
 ...
 */
export interface IEnvironmentVariableValueKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"environmentvariablevalueid"`. */
...
/** A one-to-many (or many-to-one) relationship involving the EnvironmentVariableValue table. */
export interface IEnvironmentVariableValueOneToManyRelationship {
...
/**
 * Privilege type for an EnvironmentVariableValue table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type EnvironmentVariableValuePrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the EnvironmentVariableValue table. */
export interface IEnvironmentVariableValuePrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateEnvironmentVariableValue"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: EnvironmentVariableValuePrivilegeType
...
/** A many-to-many relationship involving the EnvironmentVariableValue table. */
export interface IEnvironmentVariableValueManyToManyRelationship {
  /** OData schema name for the relationship. */
  schemaName: string
...
/**
 * A Dataverse solution that contains the EnvironmentVariableValue table as a component.
 *
 * @remarks
 ...
 */
export interface IEnvironmentVariableValueSolution {
...
/** Return value of the {@link useEnvironmentVariableValueMetadata} hook. */
export interface IUseEnvironmentVariableValueMetadataResult {
   ...
   *
   * @example `{ "value": "Value", "environmentvariablevalueid": "Environment Variable Value" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the EnvironmentVariableValue table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: IEnvironmentVariableValueAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: IEnvironmentVariableValueAttributeMetadata[]
  /**
  ...
  /**
   * Whether the EnvironmentVariableValue table is an activity table.
   * `null` while loading or on error.
   */
   ...
   */
  keyMetadata: IEnvironmentVariableValueKeyMetadata
  /**
   * All one-to-many relationships where EnvironmentVariableValue is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: IEnvironmentVariableValueOneToManyRelationship[]
  /**
   * All many-to-one relationships where EnvironmentVariableValue is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: IEnvironmentVariableValueOneToManyRelationship[]
  /**
   * All many-to-many relationships involving EnvironmentVariableValue.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: IEnvironmentVariableValueManyToManyRelationship[]
  /**
   * Security privileges defined for the EnvironmentVariableValue table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: IEnvironmentVariableValuePrivilege[]
  /**
   * Solutions that contain the EnvironmentVariableValue table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
   ...
   */
  solutions: IEnvironmentVariableValueSolution[]
...
/**
 * Fetches entity and column metadata for the Dataverse `environmentvariablevalue` table
 * via {@link EnvironmentvariablevaluesService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseEnvironmentVariableValueMetadataResult} containing metadata state,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useEnvironmentVariableValueMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['value']) // "Value"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "environmentvariablevalueid"
 * ```
 */
export function useEnvironmentVariableValueMetadata(): IUseEnvironmentVariableValueMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<IEnvironmentVariableValueSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await EnvironmentvariablevaluesService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useEnvironmentVariableValueMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useEnvironmentVariableValueMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useEnvironmentVariableValueMetadata: failed to load solution components', message)
  ...
  const attributes: IEnvironmentVariableValueAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: IEnvironmentVariableValueKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: IEnvironmentVariableValueOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: IEnvironmentVariableValueOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: IEnvironmentVariableValueManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: IEnvironmentVariableValuePrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as EnvironmentVariableValuePrivilegeType,
