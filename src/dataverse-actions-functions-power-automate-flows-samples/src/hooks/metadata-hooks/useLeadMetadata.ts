// AI-CONTEXT: Fetches and derives Lead entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/LeadsService.ts                (PAC-generated SDK client — do not edit)
...
// AI-CONSTRAINT: Never call LeadsService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the Lead table as returned by Dataverse metadata. */
export interface ILeadAttributeMetadata {
  /** Logical column name, e.g. `"leadid"` or `"fullname"`. */
  logicalName: string
  /** OData schema name, e.g. `"LeadId"` or `"FullName"`. */
  schemaName: string
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"lead"`. */
...
/**
 * Primary key metadata for the Lead table, derived from the entity metadata selects.
 *
 * @remarks
 ...
 */
export interface ILeadKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"leadid"`. */
...
/** A one-to-many (or many-to-one) relationship involving the Lead table. */
export interface ILeadOneToManyRelationship {
  /** OData schema name for the relationship, e.g. `"lead_activity_parties"`. */
  schemaName: string
...
/**
 * Privilege type for a Lead table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type LeadPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the Lead table. */
export interface ILeadPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateLead"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: LeadPrivilegeType
...
/** A many-to-many relationship involving the Lead table. */
export interface ILeadManyToManyRelationship {
...
/**
 * A Dataverse solution that contains the Lead table as a component.
 *
 * @remarks
 ...
 */
export interface ILeadSolution {
...
/** Return value of the {@link useLeadMetadata} hook. */
export interface IUseLeadMetadataResult {
   ...
   *
   * @example `{ "fullname": "Full Name", "leadid": "Lead", "companyname": "Company Name" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the Lead table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: ILeadAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: ILeadAttributeMetadata[]
  /**
   * Dataverse table type for the Lead table, e.g. `"Standard"`, `"Elastic"`, `"Virtual"`, or `"Activity"`.
   * `null` while loading or on error.
   */
  tableType: string | null
  /**
   * Whether the Lead table is an activity table.
   * `null` while loading or on error.
   */
  ...
  /**
   * Primary key metadata for the Lead table, derived from `metadata.PrimaryIdAttribute`,
   ...
   */
  keyMetadata: ILeadKeyMetadata
  /**
   * All one-to-many relationships where Lead is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: ILeadOneToManyRelationship[]
  /**
   * All many-to-one relationships where Lead is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: ILeadOneToManyRelationship[]
  /**
   * All many-to-many relationships involving Lead.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: ILeadManyToManyRelationship[]
  /**
   * Security privileges defined for the Lead table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: ILeadPrivilege[]
  /**
   * Solutions that contain the Lead table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
...
/**
 * Fetches entity and column metadata for the Dataverse `lead` table via {@link LeadsService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseLeadMetadataResult} containing metadata state, per-tab derived collections,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useLeadMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['fullname']) // "Full Name"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "leadid"
 * ```
 */
export function useLeadMetadata(): IUseLeadMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<ILeadSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await LeadsService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useLeadMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useLeadMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useLeadMetadata: failed to load solution components', message)
  ...
  const attributes: ILeadAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: ILeadKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: ILeadOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: ILeadOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: ILeadManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: ILeadPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as LeadPrivilegeType,
