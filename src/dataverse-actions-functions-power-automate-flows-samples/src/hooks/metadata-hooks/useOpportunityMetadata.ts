// AI-CONTEXT: Fetches and derives Opportunity entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/OpportunitiesService.ts        (PAC-generated SDK client — do not edit)
...
// AI-CONSTRAINT: Never call OpportunitiesService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the Opportunity table as returned by Dataverse metadata. */
export interface IOpportunityAttributeMetadata {
  /** Logical column name, e.g. `"opportunityid"` or `"name"`. */
  logicalName: string
  /** OData schema name, e.g. `"OpportunityId"` or `"Name"`. */
  schemaName: string
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"opportunity"`. */
...
/**
 * Primary key metadata for the Opportunity table, derived from the entity metadata selects.
 *
 * @remarks
 ...
 */
export interface IOpportunityKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"opportunityid"`. */
...
/** A one-to-many (or many-to-one) relationship involving the Opportunity table. */
export interface IOpportunityOneToManyRelationship {
  /** OData schema name for the relationship, e.g. `"opportunity_activity_parties"`. */
  schemaName: string
...
/**
 * Privilege type for an Opportunity table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type OpportunityPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the Opportunity table. */
export interface IOpportunityPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateOpportunity"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: OpportunityPrivilegeType
...
/** A many-to-many relationship involving the Opportunity table. */
export interface IOpportunityManyToManyRelationship {
...
/**
 * A Dataverse solution that contains the Opportunity table as a component.
 *
 * @remarks
 ...
 */
export interface IOpportunitySolution {
...
/** Return value of the {@link useOpportunityMetadata} hook. */
export interface IUseOpportunityMetadataResult {
   ...
   *
   * @example `{ "name": "Topic", "opportunityid": "Opportunity", "estimatedvalue": "Est. Revenue" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the Opportunity table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: IOpportunityAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: IOpportunityAttributeMetadata[]
  /**
   * Dataverse table type for the Opportunity table, e.g. `"Standard"`, `"Elastic"`, `"Virtual"`, or `"Activity"`.
   * `null` while loading or on error.
   */
  tableType: string | null
  /**
   * Whether the Opportunity table is an activity table.
   * `null` while loading or on error.
   */
  ...
  /**
   * Primary key metadata for the Opportunity table, derived from `metadata.PrimaryIdAttribute`,
   ...
   */
  keyMetadata: IOpportunityKeyMetadata
  /**
   * All one-to-many relationships where Opportunity is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: IOpportunityOneToManyRelationship[]
  /**
   * All many-to-one relationships where Opportunity is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: IOpportunityOneToManyRelationship[]
  /**
   * All many-to-many relationships involving Opportunity.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: IOpportunityManyToManyRelationship[]
  /**
   * Security privileges defined for the Opportunity table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: IOpportunityPrivilege[]
  /**
   * Solutions that contain the Opportunity table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
   ...
   */
  solutions: IOpportunitySolution[]
...
/**
 * Fetches entity and column metadata for the Dataverse `opportunity` table via {@link OpportunitiesService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseOpportunityMetadataResult} containing metadata state, per-tab derived collections,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useOpportunityMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['name']) // "Topic"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "opportunityid"
 * ```
 */
export function useOpportunityMetadata(): IUseOpportunityMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<IOpportunitySolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await OpportunitiesService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useOpportunityMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useOpportunityMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useOpportunityMetadata: failed to load solution components', message)
  ...
  const attributes: IOpportunityAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: IOpportunityKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: IOpportunityOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: IOpportunityOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: IOpportunityManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: IOpportunityPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as OpportunityPrivilegeType,
