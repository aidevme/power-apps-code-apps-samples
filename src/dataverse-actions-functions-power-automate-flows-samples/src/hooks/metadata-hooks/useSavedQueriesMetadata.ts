// AI-CONTEXT: Fetches and derives SavedQueries entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/SavedqueriesService.ts        (PAC-generated SDK client — do not edit)
...
// AI-CONSTRAINT: Never call SavedqueriesService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the SavedQueries table as returned by Dataverse metadata. */
export interface ISavedQueriesAttributeMetadata {
  /** Logical column name, e.g. `"name"` or `"savedqueryid"`. */
  logicalName: string
  /** OData schema name, e.g. `"Name"` or `"SavedQueryId"`. */
  schemaName: string
  /** User-localised display label for the column. */
  displayName: string
  /**
   * Dataverse attribute type name, e.g. `"StringType"`, `"BooleanType"`, `"IntegerType"`.
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"savedquery"`. */
...
/**
 * Primary key metadata for the SavedQueries table, derived from the entity metadata selects.
 *
 * @remarks
 ...
 */
export interface ISavedQueriesKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"savedqueryid"`. */
...
/** A one-to-many (or many-to-one) relationship involving the SavedQueries table. */
export interface ISavedQueriesOneToManyRelationship {
...
/**
 * Privilege type for a SavedQueries table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type SavedQueriesPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the SavedQueries table. */
export interface ISavedQueriesPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateSavedQuery"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: SavedQueriesPrivilegeType
...
/** A many-to-many relationship involving the SavedQueries table. */
export interface ISavedQueriesManyToManyRelationship {
  /** OData schema name for the relationship. */
  schemaName: string
...
/**
 * A Dataverse solution that contains the SavedQueries table as a component.
 *
 * @remarks
 ...
 */
export interface ISavedQueriesSolution {
...
/** Return value of the {@link useSavedQueriesMetadata} hook. */
export interface IUseSavedQueriesMetadataResult {
   ...
   *
   * @example `{ "name": "Name", "savedqueryid": "View", "fetchxml": "Fetch XML" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the SavedQueries table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: ISavedQueriesAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: ISavedQueriesAttributeMetadata[]
  /**
   * Dataverse table type for the SavedQueries table, e.g. `"Standard"`, `"Elastic"`, `"Virtual"`, or `"Activity"`.
   * `null` while loading or on error.
   */
  tableType: string | null
  /**
   * Whether the SavedQueries table is an activity table.
   * `null` while loading or on error.
   */
  ...
  /**
   * Primary key metadata for the SavedQueries table, derived from `metadata.PrimaryIdAttribute`,
   ...
   */
  keyMetadata: ISavedQueriesKeyMetadata
  /**
   * All one-to-many relationships where SavedQueries is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: ISavedQueriesOneToManyRelationship[]
  /**
   * All many-to-one relationships where SavedQueries is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: ISavedQueriesOneToManyRelationship[]
  /**
   * All many-to-many relationships involving SavedQueries.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: ISavedQueriesManyToManyRelationship[]
  /**
   * Security privileges defined for the SavedQueries table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: ISavedQueriesPrivilege[]
  /**
   * Solutions that contain the SavedQueries table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
   ...
   */
  solutions: ISavedQueriesSolution[]
...
/**
 * Fetches entity and column metadata for the Dataverse `savedqueries` table via {@link SavedqueriesService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseSavedQueriesMetadataResult} containing metadata state, per-tab derived collections,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useSavedQueriesMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['name']) // "Name"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "savedqueryid"
 * ```
 */
export function useSavedQueriesMetadata(): IUseSavedQueriesMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<ISavedQueriesSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await SavedqueriesService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useSavedQueriesMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useSavedQueriesMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useSavedQueriesMetadata: failed to load solution components', message)
  ...
  const attributes: ISavedQueriesAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: ISavedQueriesKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: ISavedQueriesOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: ISavedQueriesOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: ISavedQueriesManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: ISavedQueriesPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as SavedQueriesPrivilegeType,
