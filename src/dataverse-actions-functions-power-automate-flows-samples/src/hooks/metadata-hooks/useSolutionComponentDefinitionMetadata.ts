// AI-CONTEXT: Fetches and derives SolutionComponentDefinition entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/SolutioncomponentdefinitionsService.ts  (PAC-generated SDK client — do not edit)
...
// AI-CONSTRAINT: Never call SolutioncomponentdefinitionsService or fetch directly from components — all data flows through this hook.
...
import { SolutioncomponentdefinitionsService } from '../../generated/services/SolutioncomponentdefinitionsService'
...
/** Describes a single attribute on the SolutionComponentDefinition table as returned by Dataverse metadata. */
export interface ISolutionComponentDefinitionAttributeMetadata {
  /** Logical column name, e.g. `"name"` or `"solutioncomponentdefinitionid"`. */
  logicalName: string
  /** OData schema name, e.g. `"Name"` or `"SolutionComponentDefinitionId"`. */
  schemaName: string
  /** User-localised display label for the column. */
  displayName: string
  /**
   * Dataverse attribute type name, e.g. `"StringType"`, `"IntegerType"`.
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"solutioncomponentdefinition"`. */
...
/**
 * Primary key metadata for the SolutionComponentDefinition table.
 *
 * @remarks
 ...
 */
export interface ISolutionComponentDefinitionKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"solutioncomponentdefinitionid"`. */
...
/** A one-to-many (or many-to-one) relationship involving the SolutionComponentDefinition table. */
export interface ISolutionComponentDefinitionOneToManyRelationship {
...
/**
 * Privilege type for a SolutionComponentDefinition table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type SolutionComponentDefinitionPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the SolutionComponentDefinition table. */
export interface ISolutionComponentDefinitionPrivilege {
  /** Internal name of the privilege, e.g. `"prvReadSolutionComponentDefinition"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: SolutionComponentDefinitionPrivilegeType
...
/** A many-to-many relationship involving the SolutionComponentDefinition table. */
export interface ISolutionComponentDefinitionManyToManyRelationship {
  /** OData schema name for the relationship. */
  schemaName: string
...
/**
 * A Dataverse solution that contains the SolutionComponentDefinition table as a component.
 *
 * @remarks
 ...
 */
export interface ISolutionComponentDefinitionSolution {
...
/** Return value of the {@link useSolutionComponentDefinitionMetadata} hook. */
export interface IUseSolutionComponentDefinitionMetadataResult {
   ...
   *
   * @example `{ "name": "Name", "solutioncomponentdefinitionid": "Solution Component Definition" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the SolutionComponentDefinition table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: ISolutionComponentDefinitionAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: ISolutionComponentDefinitionAttributeMetadata[]
  /**
  ...
  /**
   * Whether the SolutionComponentDefinition table is an activity table.
   * `null` while loading or on error.
   */
   ...
   */
  keyMetadata: ISolutionComponentDefinitionKeyMetadata
  /**
   * All one-to-many relationships where SolutionComponentDefinition is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: ISolutionComponentDefinitionOneToManyRelationship[]
  /**
   * All many-to-one relationships where SolutionComponentDefinition is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: ISolutionComponentDefinitionOneToManyRelationship[]
  /**
   * All many-to-many relationships involving SolutionComponentDefinition.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: ISolutionComponentDefinitionManyToManyRelationship[]
  /**
   * Security privileges defined for the SolutionComponentDefinition table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: ISolutionComponentDefinitionPrivilege[]
  /**
   * Solutions that contain the SolutionComponentDefinition table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
   ...
   */
  solutions: ISolutionComponentDefinitionSolution[]
...
/**
 * Fetches entity and column metadata for the Dataverse `solutioncomponentdefinition` table
 * via {@link SolutioncomponentdefinitionsService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseSolutionComponentDefinitionMetadataResult} containing metadata state,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useSolutionComponentDefinitionMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['name']) // "Name"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "solutioncomponentdefinitionid"
 * ```
 */
export function useSolutionComponentDefinitionMetadata(): IUseSolutionComponentDefinitionMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<ISolutionComponentDefinitionSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await SolutioncomponentdefinitionsService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useSolutionComponentDefinitionMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useSolutionComponentDefinitionMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useSolutionComponentDefinitionMetadata: failed to load solution components', message)
  ...
  const attributes: ISolutionComponentDefinitionAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: ISolutionComponentDefinitionKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: ISolutionComponentDefinitionOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: ISolutionComponentDefinitionOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: ISolutionComponentDefinitionManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: ISolutionComponentDefinitionPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as SolutionComponentDefinitionPrivilegeType,
