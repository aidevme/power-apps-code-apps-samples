// AI-CONTEXT: Fetches and derives SolutionComponent entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/SolutioncomponentsService.ts  (PAC-generated SDK client — do not edit)
...
// AI-CONSTRAINT: Never call SolutioncomponentsService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the SolutionComponent table as returned by Dataverse metadata. */
export interface ISolutionComponentAttributeMetadata {
  /** Logical column name, e.g. `"objectid"` or `"solutioncomponentid"`. */
  logicalName: string
  /** OData schema name, e.g. `"ObjectId"` or `"SolutionComponentId"`. */
  schemaName: string
  /** User-localised display label for the column. */
  displayName: string
  /**
   * Dataverse attribute type name, e.g. `"UniqueidentifierType"`, `"IntegerType"`.
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"solutioncomponent"`. */
...
/**
 * Primary key metadata for the SolutionComponent table.
 *
 * @remarks
 ...
 */
export interface ISolutionComponentKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"solutioncomponentid"`. */
...
/** A one-to-many (or many-to-one) relationship involving the SolutionComponent table. */
export interface ISolutionComponentOneToManyRelationship {
...
/**
 * Privilege type for a SolutionComponent table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type SolutionComponentPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the SolutionComponent table. */
export interface ISolutionComponentPrivilege {
  /** Internal name of the privilege, e.g. `"prvReadSolutionComponent"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: SolutionComponentPrivilegeType
...
/** A many-to-many relationship involving the SolutionComponent table. */
export interface ISolutionComponentManyToManyRelationship {
  /** OData schema name for the relationship. */
  schemaName: string
...
/**
 * A Dataverse solution that contains the SolutionComponent table as a component.
 *
 * @remarks
 ...
 */
export interface ISolutionComponentSolution {
...
/** Return value of the {@link useSolutionComponentMetadata} hook. */
export interface IUseSolutionComponentMetadataResult {
   ...
   *
   * @example `{ "objectid": "Regarding", "solutioncomponentid": "Solution Component" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the SolutionComponent table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: ISolutionComponentAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: ISolutionComponentAttributeMetadata[]
  /**
  ...
  /**
   * Whether the SolutionComponent table is an activity table.
   * `null` while loading or on error.
   */
   ...
   */
  keyMetadata: ISolutionComponentKeyMetadata
  /**
   * All one-to-many relationships where SolutionComponent is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: ISolutionComponentOneToManyRelationship[]
  /**
   * All many-to-one relationships where SolutionComponent is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: ISolutionComponentOneToManyRelationship[]
  /**
   * All many-to-many relationships involving SolutionComponent.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: ISolutionComponentManyToManyRelationship[]
  /**
   * Security privileges defined for the SolutionComponent table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: ISolutionComponentPrivilege[]
  /**
   * Solutions that contain the SolutionComponent table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
   ...
   */
  solutions: ISolutionComponentSolution[]
...
/**
 * Fetches entity and column metadata for the Dataverse `solutioncomponent` table
 * via {@link SolutioncomponentsService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseSolutionComponentMetadataResult} containing metadata state,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useSolutionComponentMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['objectid']) // "Regarding"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "solutioncomponentid"
 * ```
 */
export function useSolutionComponentMetadata(): IUseSolutionComponentMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<ISolutionComponentSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await SolutioncomponentsService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useSolutionComponentMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useSolutionComponentMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useSolutionComponentMetadata: failed to load solution components', message)
  ...
  const attributes: ISolutionComponentAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: ISolutionComponentKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: ISolutionComponentOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: ISolutionComponentOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: ISolutionComponentManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: ISolutionComponentPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as SolutionComponentPrivilegeType,
