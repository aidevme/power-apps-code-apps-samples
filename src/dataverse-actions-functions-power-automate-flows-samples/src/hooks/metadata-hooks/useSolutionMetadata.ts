// AI-CONTEXT: Fetches and derives Solution entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/SolutionsService.ts         (PAC-generated SDK client — do not edit)
...
// AI-CONSTRAINT: Never call SolutionsService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the Solution table as returned by Dataverse metadata. */
export interface ISolutionAttributeMetadata {
  /** Logical column name, e.g. `"uniquename"` or `"solutionid"`. */
  logicalName: string
  /** OData schema name, e.g. `"UniqueName"` or `"SolutionId"`. */
  schemaName: string
  /** User-localised display label for the column. */
  displayName: string
  /**
   * Dataverse attribute type name, e.g. `"StringType"`, `"BooleanType"`.
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"solution"`. */
...
/**
 * Primary key metadata for the Solution table.
 *
 * @remarks
 ...
 */
export interface ISolutionKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"solutionid"`. */
...
/** A one-to-many (or many-to-one) relationship involving the Solution table. */
export interface ISolutionOneToManyRelationship {
...
/**
 * Privilege type for a Solution table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type SolutionPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the Solution table. */
export interface ISolutionPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateSolution"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: SolutionPrivilegeType
...
/** A many-to-many relationship involving the Solution table. */
export interface ISolutionManyToManyRelationship {
  /** OData schema name for the relationship. */
  schemaName: string
...
/**
 * A Dataverse solution that contains the Solution table as a component.
 *
 * @remarks
 ...
 */
export interface ISolutionSolution {
...
/** Return value of the {@link useSolutionMetadata} hook. */
export interface IUseSolutionMetadataResult {
   ...
   *
   * @example `{ "uniquename": "Name", "solutionid": "Solution" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the Solution table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: ISolutionAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: ISolutionAttributeMetadata[]
  /**
  ...
  /**
   * Whether the Solution table is an activity table.
   * `null` while loading or on error.
   */
   ...
   */
  keyMetadata: ISolutionKeyMetadata
  /**
   * All one-to-many relationships where Solution is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: ISolutionOneToManyRelationship[]
  /**
   * All many-to-one relationships where Solution is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: ISolutionOneToManyRelationship[]
  /**
   * All many-to-many relationships involving Solution.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: ISolutionManyToManyRelationship[]
  /**
   * Security privileges defined for the Solution table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: ISolutionPrivilege[]
  /**
   * Solutions that contain the Solution table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
...
/**
 * Fetches entity and column metadata for the Dataverse `solution` table
 * via {@link SolutionsService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseSolutionMetadataResult} containing metadata state,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useSolutionMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['uniquename']) // "Name"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "solutionid"
 * ```
 */
export function useSolutionMetadata(): IUseSolutionMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<ISolutionSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await SolutionsService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useSolutionMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useSolutionMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useSolutionMetadata: failed to load solution components', message)
  ...
  const attributes: ISolutionAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: ISolutionKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: ISolutionOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: ISolutionOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: ISolutionManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: ISolutionPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as SolutionPrivilegeType,
