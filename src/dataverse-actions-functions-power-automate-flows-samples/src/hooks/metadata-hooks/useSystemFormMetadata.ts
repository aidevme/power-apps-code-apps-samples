// AI-CONTEXT: Fetches and derives SystemForm entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/SystemformsService.ts             (PAC-generated SDK client — do not edit)
...
// AI-CONSTRAINT: Never call SystemformsService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the SystemForm table as returned by Dataverse metadata. */
export interface ISystemFormAttributeMetadata {
  /** Logical column name, e.g. `"name"` or `"formid"`. */
  logicalName: string
  /** OData schema name, e.g. `"Name"` or `"FormId"`. */
  schemaName: string
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"systemform"`. */
...
/**
 * Primary key metadata for the SystemForm table, derived from the entity metadata selects.
 *
 * @remarks
 ...
 */
export interface ISystemFormKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"formid"`. */
...
/** A one-to-many (or many-to-one) relationship involving the SystemForm table. */
export interface ISystemFormOneToManyRelationship {
  /** OData schema name for the relationship, e.g. `"form_ancestor_form"`. */
  schemaName: string
...
/**
 * Privilege type for a SystemForm table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type SystemFormPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the SystemForm table. */
export interface ISystemFormPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateSystemForm"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: SystemFormPrivilegeType
...
/** A many-to-many relationship involving the SystemForm table. */
export interface ISystemFormManyToManyRelationship {
...
/**
 * A Dataverse solution that contains the SystemForm table as a component.
 *
 * @remarks
 ...
 */
export interface ISystemFormSolution {
...
/** Return value of the {@link useSystemFormMetadata} hook. */
export interface IUseSystemFormMetadataResult {
   ...
   *
   * @example `{ "name": "Name", "formid": "Form", "objecttypecode": "Entity Name" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the SystemForm table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: ISystemFormAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: ISystemFormAttributeMetadata[]
  /**
   * Dataverse table type for the SystemForm table, e.g. `"Standard"`.
   * `null` while loading or on error.
   */
  tableType: string | null
  /**
   * Whether the SystemForm table is an activity table.
   * `null` while loading or on error.
   */
  ...
  /**
   * Primary key metadata for the SystemForm table, derived from `metadata.PrimaryIdAttribute`,
   ...
   */
  keyMetadata: ISystemFormKeyMetadata
  /**
   * All one-to-many relationships where SystemForm is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: ISystemFormOneToManyRelationship[]
  /**
   * All many-to-one relationships where SystemForm is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: ISystemFormOneToManyRelationship[]
  /**
   * All many-to-many relationships involving SystemForm.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: ISystemFormManyToManyRelationship[]
  /**
   * Security privileges defined for the SystemForm table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: ISystemFormPrivilege[]
  /**
   * Solutions that contain the SystemForm table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
...
/**
 * Fetches entity and column metadata for the Dataverse `systemform` table via {@link SystemformsService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseSystemFormMetadataResult} containing metadata state, per-tab derived collections,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useSystemFormMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['name']) // "Name"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "formid"
 * ```
 */
export function useSystemFormMetadata(): IUseSystemFormMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<ISystemFormSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await SystemformsService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useSystemFormMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useSystemFormMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useSystemFormMetadata: failed to load solution components', message)
  ...
  const attributes: ISystemFormAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: ISystemFormKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: ISystemFormOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: ISystemFormOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: ISystemFormManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: ISystemFormPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as SystemFormPrivilegeType,
