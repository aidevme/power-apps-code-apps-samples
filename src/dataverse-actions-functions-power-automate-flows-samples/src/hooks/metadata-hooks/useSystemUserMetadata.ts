// AI-CONTEXT: Fetches and derives SystemUser entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/SystemusersService.ts             (PAC-generated SDK client — do not edit)
...
// AI-CONSTRAINT: Never call SystemusersService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the SystemUser table as returned by Dataverse metadata. */
export interface ISystemUserAttributeMetadata {
  /** Logical column name, e.g. `"fullname"` or `"systemuserid"`. */
  logicalName: string
  /** OData schema name, e.g. `"FullName"` or `"SystemUserId"`. */
  schemaName: string
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"systemuser"`. */
...
/**
 * Primary key metadata for the SystemUser table, derived from the entity metadata selects.
 *
 * @remarks
 ...
 */
export interface ISystemUserKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"systemuserid"`. */
...
/** A one-to-many (or many-to-one) relationship involving the SystemUser table. */
export interface ISystemUserOneToManyRelationship {
  /** OData schema name for the relationship, e.g. `"systemuser_accounts"`. */
  schemaName: string
...
/**
 * Privilege type for a SystemUser table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type SystemUserPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the SystemUser table. */
export interface ISystemUserPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateSystemUser"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: SystemUserPrivilegeType
...
/** A many-to-many relationship involving the SystemUser table. */
export interface ISystemUserManyToManyRelationship {
...
/**
 * A Dataverse solution that contains the SystemUser table as a component.
 *
 * @remarks
 ...
 */
export interface ISystemUserSolution {
...
/** Return value of the {@link useSystemUserMetadata} hook. */
export interface IUseSystemUserMetadataResult {
   ...
   *
   * @example `{ "fullname": "Full Name", "systemuserid": "User", "businessunitid": "Business Unit" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the SystemUser table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: ISystemUserAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: ISystemUserAttributeMetadata[]
  /**
   * Dataverse table type for the SystemUser table, e.g. `"Standard"`.
   * `null` while loading or on error.
   */
  tableType: string | null
  /**
   * Whether the SystemUser table is an activity table.
   * `null` while loading or on error.
   */
  ...
  /**
   * Primary key metadata for the SystemUser table, derived from `metadata.PrimaryIdAttribute`,
   ...
   */
  keyMetadata: ISystemUserKeyMetadata
  /**
   * All one-to-many relationships where SystemUser is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: ISystemUserOneToManyRelationship[]
  /**
   * All many-to-one relationships where SystemUser is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: ISystemUserOneToManyRelationship[]
  /**
   * All many-to-many relationships involving SystemUser.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: ISystemUserManyToManyRelationship[]
  /**
   * Security privileges defined for the SystemUser table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: ISystemUserPrivilege[]
  /**
   * Solutions that contain the SystemUser table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
...
/**
 * Fetches entity and column metadata for the Dataverse `systemuser` table via {@link SystemusersService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseSystemUserMetadataResult} containing metadata state, per-tab derived collections,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useSystemUserMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['fullname']) // "Full Name"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "systemuserid"
 * ```
 */
export function useSystemUserMetadata(): IUseSystemUserMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<ISystemUserSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await SystemusersService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useSystemUserMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useSystemUserMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useSystemUserMetadata: failed to load solution components', message)
  ...
  const attributes: ISystemUserAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: ISystemUserKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: ISystemUserOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: ISystemUserOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: ISystemUserManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: ISystemUserPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as SystemUserPrivilegeType,
