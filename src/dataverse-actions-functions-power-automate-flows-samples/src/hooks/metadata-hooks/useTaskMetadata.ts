...
export interface ITaskAttributeMetadata {
  ...
  displayName: string
  /** Dataverse attribute type, e.g. `"StringType"`, `"DateTimeType"`, `"PicklistType"`. */
...
/**
 * Fetches entity and column metadata for the Task table via {@link TasksService.getMetadata}.
 *
 ...
 * @example
 * ```tsx
 * const { columnDisplayNames, requiredAttributes, loading } = useTaskMetadata()
 * if (!loading) console.log(columnDisplayNames['subject']) // "Subject"
 * ```
 */
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await TasksService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all' } })
        if (cancelled) return
        if (!result.success) {
