// AI-CONTEXT: Fetches and derives Task entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/TasksService.ts             (PAC-generated SDK client — do not edit)
...
// AI-CONSTRAINT: Never call TasksService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the Task table as returned by Dataverse metadata. */
export interface ITaskAttributeMetadata {
  /** Logical column name, e.g. `"subject"` or `"activityid"`. */
  logicalName: string
  /** OData schema name, e.g. `"Subject"` or `"ActivityId"`. */
  schemaName: string
  /** User-localised display label for the column. */
  displayName: string
  /**
   * Dataverse attribute type name, e.g. `"StringType"`, `"DateTimeType"`, `"PicklistType"`.
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"task"`. */
...
/**
 * Primary key metadata for the Task table, derived from the entity metadata selects.
 *
 * @remarks
 ...
 */
export interface ITaskKeyMetadata {
...
/** A one-to-many (or many-to-one) relationship involving the Task table. */
export interface ITaskOneToManyRelationship {
  /** OData schema name for the relationship, e.g. `"task_activity_parties"`. */
  schemaName: string
...
/**
 * Privilege type for a Task table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type TaskPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the Task table. */
export interface ITaskPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateTask"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: TaskPrivilegeType
...
/** A many-to-many relationship involving the Task table. */
export interface ITaskManyToManyRelationship {
...
/**
 * A Dataverse solution that contains the Task table as a component.
 *
 * @remarks
...
/** Return value of the {@link useTaskMetadata} hook. */
export interface IUseTaskMetadataResult {
   ...
   *
   * @example `{ "subject": "Subject", "activityid": "Activity", "scheduledend": "Due Date" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the Task table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: ITaskAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: ITaskAttributeMetadata[]
  /**
   * Dataverse table type for the Task table, e.g. `"Standard"`, `"Elastic"`, `"Virtual"`, or `"Activity"`.
   * `null` while loading or on error.
   */
  tableType: string | null
  /**
   * Whether the Task table is an activity table.
   * `null` while loading or on error.
   */
  ...
  /**
   * Primary key metadata for the Task table, derived from `metadata.PrimaryIdAttribute`,
   ...
   */
  keyMetadata: ITaskKeyMetadata
  /**
   * All one-to-many relationships where Task is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: ITaskOneToManyRelationship[]
  /**
   * All many-to-one relationships where Task is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: ITaskOneToManyRelationship[]
  /**
   * All many-to-many relationships involving Task.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: ITaskManyToManyRelationship[]
  /**
   * Security privileges defined for the Task table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: ITaskPrivilege[]
  /**
   * Solutions that contain the Task table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
...
/**
 * Fetches entity and column metadata for the Dataverse `task` table via {@link TasksService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseTaskMetadataResult} containing metadata state, per-tab derived collections,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useTaskMetadata()
 ...
 * ```
 */
export function useTaskMetadata(): IUseTaskMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<ITaskSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await TasksService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useTaskMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useTaskMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useTaskMetadata: failed to load solution components', message)
  ...
  const attributes: ITaskAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: ITaskKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: ITaskOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: ITaskOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: ITaskManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: ITaskPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as TaskPrivilegeType,
