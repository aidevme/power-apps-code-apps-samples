// AI-CONTEXT: Fetches and derives Team entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/TeamsService.ts              (PAC-generated SDK client — do not edit)
...
// AI-CONSTRAINT: Never call TeamsService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the Team table as returned by Dataverse metadata. */
export interface ITeamAttributeMetadata {
  /** Logical column name, e.g. `"name"` or `"teamid"`. */
  logicalName: string
  /** OData schema name, e.g. `"Name"` or `"TeamId"`. */
  schemaName: string
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"team"`. */
...
/**
 * Primary key metadata for the Team table, derived from the entity metadata selects.
 *
 * @remarks
 ...
 */
export interface ITeamKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"teamid"`. */
...
/** A one-to-many (or many-to-one) relationship involving the Team table. */
export interface ITeamOneToManyRelationship {
...
/**
 * Privilege type for a Team table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type TeamPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the Team table. */
export interface ITeamPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateTeam"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: TeamPrivilegeType
...
/** A many-to-many relationship involving the Team table. */
export interface ITeamManyToManyRelationship {
  /** OData schema name for the relationship. */
  schemaName: string
...
/**
 * A Dataverse solution that contains the Team table as a component.
 *
 * @remarks
 ...
 */
export interface ITeamSolution {
...
/** Return value of the {@link useTeamMetadata} hook. */
export interface IUseTeamMetadataResult {
   ...
   *
   * @example `{ "name": "Team Name", "teamid": "Team", "businessunitid": "Business Unit" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the Team table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: ITeamAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: ITeamAttributeMetadata[]
  /**
   * Dataverse table type for the Team table, e.g. `"Standard"`, `"Elastic"`, `"Virtual"`, or `"Activity"`.
   * `null` while loading or on error.
   */
  tableType: string | null
  /**
   * Whether the Team table is an activity table.
   * `null` while loading or on error.
   */
  ...
  /**
   * Primary key metadata for the Team table, derived from `metadata.PrimaryIdAttribute`,
   ...
   */
  keyMetadata: ITeamKeyMetadata
  /**
   * All one-to-many relationships where Team is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: ITeamOneToManyRelationship[]
  /**
   * All many-to-one relationships where Team is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: ITeamOneToManyRelationship[]
  /**
   * All many-to-many relationships involving Team.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: ITeamManyToManyRelationship[]
  /**
   * Security privileges defined for the Team table.
   * Empty array while loading or on error.
   *
  ...
  /**
   * Solutions that contain the Team table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
...
/**
 * Fetches entity and column metadata for the Dataverse `team` table via {@link TeamsService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseTeamMetadataResult} containing metadata state, per-tab derived collections,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useTeamMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['name']) // "Team Name"
 * console.log(attributes.find(a => a.isPrimaryId)?.logicalName) // "teamid"
 * ```
 */
export function useTeamMetadata(): IUseTeamMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<ITeamSolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await TeamsService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useTeamMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useTeamMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useTeamMetadata: failed to load solution components', message)
  ...
  const attributes: ITeamAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: ITeamKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: ITeamOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: ITeamOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: ITeamManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: ITeamPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as TeamPrivilegeType,
