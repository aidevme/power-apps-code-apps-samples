// AI-CONTEXT: Fetches and derives TransactionCurrency entity metadata from Dataverse — attributes, relationships, privileges, and solutions.
// AI-FILE-RELATIONS:
//   - service:  src/generated/services/TransactioncurrenciesService.ts  (PAC-generated SDK client — do not edit)
...
// AI-CONSTRAINT: Never call TransactioncurrenciesService or fetch directly from components — all data flows through this hook.
...
/** Describes a single attribute on the TransactionCurrency table as returned by Dataverse metadata. */
export interface ITransactionCurrencyAttributeMetadata {
  /** Logical column name, e.g. `"transactioncurrencyid"` or `"currencyname"`. */
  logicalName: string
  /** OData schema name, e.g. `"TransactionCurrencyId"` or `"CurrencyName"`. */
  schemaName: string
  ...
  description: string
  /** Logical name of the entity that owns this attribute, e.g. `"transactioncurrency"`. */
   ...
   */
  sourceType: number
  /** Whether the attribute can be customised (`IsCustomizable.Value`). */
  isCustomizable: boolean
  /** Whether the attribute holds a data-source secret value. */
  isDataSourceSecret: boolean
  /** Whether the attribute is enabled as a global filter (`IsGlobalFilterEnabled.Value`). */
  isGlobalFilterEnabled: boolean
  /** Whether the attribute is a logical attribute not stored in the main table. */
  isLogical: boolean
  /** Whether the attribute can be renamed (`IsRenameable.Value`). */
  isRenameable: boolean
  /** Whether the attribute is retrievable via the Web API (`IsRetrievable`). */
  isRetrievable: boolean
  /** Whether the attribute supports server-side sorting (`IsSortableEnabled.Value`). */
  isSortableEnabled: boolean
}

/**
 * Primary key metadata for the TransactionCurrency table.
 *
 * @remarks
 ...
 */
export interface ITransactionCurrencyKeyMetadata {
  /** Logical name of the primary ID attribute, e.g. `"transactioncurrencyid"`. */
  primaryIdAttribute: string | null
  /** Logical name of the primary name attribute, e.g. `"currencyname"`. */
  primaryNameAttribute: string | null
  /** All column logical names that form the primary key. */
  primaryKey: string[]
}

/** A one-to-many (or many-to-one) relationship involving the TransactionCurrency table. */
export interface ITransactionCurrencyOneToManyRelationship {
  /** OData schema name for the relationship, e.g. `"transactioncurrency_account"`. */
  schemaName: string
...
/**
 * Privilege type for a TransactionCurrency table security privilege.
 *
 * @remarks
 * Maps to the Dataverse `PrivilegeType` enum.
 * `'None'` is included for completeness but is not normally returned.
 */
export type TransactionCurrencyPrivilegeType = 'None' | 'Create' | 'Read' | 'Write' | 'Delete' | 'Assign' | 'Share' | 'Append' | 'AppendTo'

/** A security privilege associated with the TransactionCurrency table. */
export interface ITransactionCurrencyPrivilege {
  /** Internal name of the privilege, e.g. `"prvCreateTransactionCurrency"`. */
  name: string
  /** Unique identifier of the privilege record. */
  privilegeId: string
  /** The CRUD-style action this privilege controls. */
  privilegeType: TransactionCurrencyPrivilegeType
...
/** A many-to-many relationship involving the TransactionCurrency table. */
export interface ITransactionCurrencyManyToManyRelationship {
...
/**
 * A Dataverse solution that contains the TransactionCurrency table as a component.
 *
 * @remarks
 ...
 */
export interface ITransactionCurrencySolution {
...
/** Return value of the {@link useTransactionCurrencyMetadata} hook. */
export interface IUseTransactionCurrencyMetadataResult {
   ...
   *
   * @example `{ "currencyname": "Currency Name", "transactioncurrencyid": "Transaction Currency" }`
   */
  columnDisplayNames: Record<string, string>
  /**
   * Metadata for every attribute on the TransactionCurrency table, derived from `metadata.Attributes`.
   * Empty array while loading or on error.
   */
  attributes: ITransactionCurrencyAttributeMetadata[]
  /**
   ...
   */
  requiredAttributes: ITransactionCurrencyAttributeMetadata[]
  /**
   * Dataverse table type for the TransactionCurrency table, e.g. `"Standard"`.
   * `null` while loading or on error.
   */
  tableType: string | null
  /**
   * Whether the TransactionCurrency table is an activity table.
   * `null` while loading or on error.
   */
   ...
   */
  keyMetadata: ITransactionCurrencyKeyMetadata
  /**
   * All one-to-many relationships where TransactionCurrency is the referenced ("one") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.OneToManyRelationships`.
   */
  oneToManyRelationships: ITransactionCurrencyOneToManyRelationship[]
  /**
   * All many-to-one relationships where TransactionCurrency is the referencing ("many") side.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToOneRelationships`.
   */
  manyToOneRelationships: ITransactionCurrencyOneToManyRelationship[]
  /**
   * All many-to-many relationships involving TransactionCurrency.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.ManyToManyRelationships`.
   */
  manyToManyRelationships: ITransactionCurrencyManyToManyRelationship[]
  /**
   * Security privileges defined for the TransactionCurrency table.
   * Empty array while loading or on error.
   *
   * @remarks Derived from `metadata.Privileges`.
   */
  privileges: ITransactionCurrencyPrivilege[]
  /**
   * Solutions that contain the TransactionCurrency table as a component.
   * Empty array while loading or on error.
   *
   * @remarks
   ...
   */
  solutions: ITransactionCurrencySolution[]
...
/**
 * Fetches entity and column metadata for the Dataverse `transactioncurrency` table via {@link TransactioncurrenciesService.getMetadata}.
 *
 * @remarks
 * **Two-effect pattern:**
 *
 ...
 *
 * @returns {@link IUseTransactionCurrencyMetadataResult} containing metadata state, per-tab derived collections,
 ...
 * @example
 * ```tsx
 * const { attributes, columnDisplayNames, loading, error, retry } = useTransactionCurrencyMetadata()
 * if (loading) return <Spinner />
 * if (error) return <button onClick={retry}>Retry</button>
 * console.log(columnDisplayNames['currencyname']) // "Currency Name"
 * ```
 */
export function useTransactionCurrencyMetadata(): IUseTransactionCurrencyMetadataResult {
  const [metadata, setMetadata] = useState<Partial<EntityMetadata> | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [attempt, setAttempt] = useState(0)
  const [solutions, setSolutions] = useState<ITransactionCurrencySolution[]>([])
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await TransactioncurrenciesService.getMetadata({ metadata: ALL_ENTITY_METADATA_FIELDS, schema: { columns: 'all', oneToMany: true, manyToOne: true, manyToMany: true } })
        if (cancelled) return
        if (!result.success) {
          const message = result.error?.message ?? 'getMetadata returned a failure result'
          console.error('useTransactionCurrencyMetadata:', message, result.error)
          setError(message)
          return
        }
        ...
        if (!cancelled) {
          ...
          console.error('useTransactionCurrencyMetadata: failed to load metadata', message)
          setError(message)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchSolutions = async () => {
      try {
        ...
        if (!cancelled) {
          const message = err instanceof Error ? err.message : String(err)
          console.error('useTransactionCurrencyMetadata: failed to load solution components', message)
  ...
  const attributes: ITransactionCurrencyAttributeMetadata[] = metadata?.Attributes
  ...
  const keyMetadata: ITransactionCurrencyKeyMetadata = {
    primaryIdAttribute: metadata?.PrimaryIdAttribute ?? null,
    primaryNameAttribute: metadata?.PrimaryNameAttribute ?? null,
    primaryKey: metadata?.PrimaryKey ?? [],
  }

  const oneToManyRelationships: ITransactionCurrencyOneToManyRelationship[] = metadata?.OneToManyRelationships
  ...
  const manyToOneRelationships: ITransactionCurrencyOneToManyRelationship[] = metadata?.ManyToOneRelationships
  ...
  const manyToManyRelationships: ITransactionCurrencyManyToManyRelationship[] = metadata?.ManyToManyRelationships
  ...
  const privileges: ITransactionCurrencyPrivilege[] = metadata?.Privileges
    ? metadata.Privileges.map(p => ({
        ...
        privilegeType: (p.PrivilegeType != null ? (getPrivilegeTypeName(p.PrivilegeType) ?? String(p.PrivilegeType)) : 'None') as TransactionCurrencyPrivilegeType,
  ...
  return {
    ...
    columnDisplayNames, attributes, requiredAttributes,
    tableType, isActivity, tableInfo, keyMetadata,
    oneToManyRelationships, manyToOneRelationships, manyToManyRelationships,
    privileges, solutions, solutionsLoading, solutionsError,
